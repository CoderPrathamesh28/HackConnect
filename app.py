...
def create_hackathon():
    ...
    if request.method == 'POST':
        ...
        if not title or not description or not start_date or not end_date:
            ...
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO hackathons (title, description, start_date, end_date, organizer_id, status, theme, prizes)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (title, description, start_date, end_date, g.user['id'], status, theme, prizes))
...
def edit_hackathon(id):
    ...
    if request.method == 'POST':
        ...
        if not title or not description or not start_date or not end_date:
            ...
            conn.execute('''
                UPDATE hackathons 
                SET title = ?, description = ?, start_date = ?, end_date = ?, theme = ?, prizes = ?, status = ?
                WHERE id = ?
            ''', (title, description, start_date, end_date, theme, prizes, status, id))
            conn.commit()
